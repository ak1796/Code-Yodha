...
const STATUS_CONFIG = {
  filed: { label: 'TL_ComplaintFiled', color: 'bg-saffron-light text-saffron' },
  assigned: { label: 'TL_OfficerAssigned', color: 'bg-amber-light text-amber' },
  in_progress: { label: 'TL_WorkInProgress', color: 'bg-blue-50 text-blue-600' },
  resolved: { label: 'TL_IssueResolved', color: 'bg-emerald-light text-emerald' },
  rejected: { label: 'StatusRejected', color: 'bg-gray-100 text-gray-500' }
};
      ...
      {ticket.status !== 'resolved' && ticket.sla_deadline && (
        <div className="px-5 py-2 bg-gray-50 border-t border-border flex items-center justify-between">
           <div className="flex items-center gap-1 text-[10px] text-text-secondary uppercase tracking-tight">
             <Clock size={10} /> {t('ResolvingBy')} {formatDate(ticket.sla_deadline)}
           </div>
        </div>
      )}
    </div>

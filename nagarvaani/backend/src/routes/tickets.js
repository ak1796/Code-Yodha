...
router.patch('/:id/resolve', authenticate, isOfficer, upload.any(), async (req, res) => {
  ...
  try {
    ...
    if (fetchErr || !ticket) {
      console.error("❌ Node Retrieval Failure:", fetchErr);
      return res.status(404).json({ error: 'Signal node not found in jurisdictional ledger' });
    }
    ...
    if (beforeFile) {
      ...
      try {
        ...
        console.error("❌ Before Image Upload Exception:", e.message);
      }
    }
    ...
    if (afterFile) {
      ...
      try {
        ...
        console.error("❌ After Image Upload Exception:", e.message);
      }
    }

    console.log("🏛️ Finalizing Ledger Update...");
    ...
    if (updateError) {
      console.error("❌ Database Update Failure:", updateError);
      throw updateError;
    }
    ...
    console.error("❌ Fatal Resolve Error Handler:", error);
    res.status(500).json({ 
      error: 'Jurisdictional State failure', 
      message: error.message,
      stack: error.stack,
...
router.post('/:id/review', async (req, res) => {
  ...
  try {
    ...
    if (error) {
      console.error("❌ Review Persistence Failure:", error.message);

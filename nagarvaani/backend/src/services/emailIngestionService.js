...
async function findOfficerForComplaint(department, city) {
  ...
  if (error) {
    console.error("❌ Finder Failure:", error.message);
    return null;
  }
  return officer;
}
          ...
          f.on('message', (msg, seqno) => {
            msg.on('body', (stream, info) => {
              simpleParser(stream, async (err, mail) => {
                if (err) return console.error("❌ Mail Parse Failure:", err);
                ...
                if (senderEmail === process.env.GMAIL_USER || senderEmail.includes('mailer-daemon') || senderEmail.includes('noreply')) {
                  console.log(`⏭️ Skipping non-citizen signal from ${senderEmail}`);
                  return;
                }
                ...
                try {
                  ...
                  console.log(`✅ Processed Signal ${ticket.id.substring(0,8)} | Assigned: ${officer ? officer.full_name : 'N/A'}`);

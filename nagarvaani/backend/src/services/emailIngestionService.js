...
async function findOfficerForComplaint(department, city) {
  ...
  if (error) {
    console.error("Ã”Ã˜Ã® Finder Failure:", error.message);
    return null;
  }
  return officer;
}
          ...
          f.on('message', (msg, seqno) => {
            msg.on('body', (stream, info) => {
              simpleParser(stream, async (err, mail) => {
                if (err) return console.error("Ã”Ã˜Ã® Mail Parse Failure:", err);
                ...
                try {
                  ...
                  console.log(`Ã”Â£Ã  Processed Signal ${ticket.id.substring(0,8)} | Assigned: ${officer ? officer.full_name : 'N/A'}`);

                } catch (err) {
                  console.error("Ã”Ã˜Ã® Signal Processing Error:", err.message);
                }
              });
            });
          });
